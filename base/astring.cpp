...
AString::AString(const char* str)
{
	if (str != NULL)
	{
		buffer_.pushBack(str, strlen(str), true);
		buffer_.pushBack((unsigned char)0, 1, true);
	}
}
...
void AString::format(const char* sformat, ...)
{
	...
	if (wlen > 0)
		buffer_.pushBack(buf, wlen, true);
}
...
AString AString::mid(int iFirst, int iCount)
{
	...
	{
		str.buffer_.pushBack(iCount, true);
		memcpy(str.buffer_.beginRead(), string()+iFirst, iCount);
		str.buffer_.pushBack((unsigned char)0, 1, true);
	}
