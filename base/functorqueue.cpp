...
void FunctorQueue::post(const Functor& func)
{
	Lock lock(mutex_);
	actQueue_.push_back(func);
}

UInt32 FunctorQueue::post(const Functor& func, MicroSecond delay)
{
	if (delay <= 0)
	{
		post(func);
	...
	TimerQueue::iterator it = tmQueue_.insert(std::pair<Timestamp, TmFunctor>(Timestamp::NOW()+MicroSecond(delay), TmFunctor(sequence_, func)));
...
MicroSecond FunctorQueue::run()
{
	...
	if (!tmQueue_.empty())
	{
		...
		for (; it!=tmQueue_.end(); ++it)
		{
			...
			if (delay > 0)
				break;
			actQueue_.push_back(it->second.func_);
